...
def plot_loss(experiments, output_dir, smooth=0):
    ...
    for idx, exp in enumerate(experiments):
        ...
        y = rolling_smooth(df["loss"], smooth) if smooth > 0 else df["loss"]
        style = EXP_LINESTYLES[idx % len(EXP_LINESTYLES)]
        ax.plot(df["episode"], y, style[0], label=label, **style[1])
    ax.set_xlabel("Episode")
    ax.set_ylabel("Loss")
    ax.set_title("Loss vs Episode")
    ax.set_yscale("log")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    path = os.path.join(output_dir, "loss.pdf")
